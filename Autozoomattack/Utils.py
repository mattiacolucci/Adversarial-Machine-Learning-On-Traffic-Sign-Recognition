...
def load_AE(codec_prefix, print_summary=False):
    ...
    decoder = tf.keras.models.model_from_json(json_file.read(), custom_objects={"tf": tf})
    json_file.close()
...
def load_codec(codec_prefix, print_summary=False):
    ...
    encoder = tf.keras.models.model_from_json(json_file.read(), custom_objects={"tf": tf})
    json_file.close()
    ...
    if not os.path.isfile(decoder_model_filename):
        raise Exception("The file for decoder model does not exist:{}".format(decoder_model_filename))
    json_file = open(decoder_model_filename, 'r')
    decoder_temp = tf.keras.models.model_from_json(json_file.read(), custom_objects={"tf": tf})
    json_file.close()
    ...
    decoder = tf.keras.models.Sequential.from_config(config2, custom_objects={"tf": tf})
